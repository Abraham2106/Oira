...
**Alcance:** NotaLocal, aplicación Electron local de documentación clínica. Este informe no prueba la compatibilidad, el rendimiento ni el empaquetado en una máquina concreta.
...
**Implicación:** la primera reproducción debe ser literal. No se añadirán SQLite, módulos nativos adicionales, APIs de NotaLocal ni una abstracción nueva antes de que el scaffold corra y empaquete.
...
**Decisión arquitectónica provisional:** NotaLocal conserva su regla interna: el único import de QVAC vivirá bajo src/main/qvac y el renderer accederá solo a operaciones permitidas de window.notalocal. La regla es nuestra arquitectura; no debe presentarse como una API adicional de QVAC.
...
La misma página permite comprobar un subconjunto mediante qvac doctor y su salida JSON. Declara al menos 2 GB de RAM total, recomienda 4 GB y advierte que bajo 4 GB la mayoría de LLM no cargará. No se convierte esa cifra en un presupuesto de NotaLocal: el modelo, los workers y la coexistencia con STT pertenecen al laboratorio R-4.
...
No afirmar que QVAC funciona en todos los equipos, que NotaLocal se probó en los tres sistemas, que se fijaron versiones compatibles, que funciona offline desde el primer inicio, ni que asar desactivado protege datos. Tampoco llamar seguro, firmado o listo para producción a un artefacto no ejecutado.

## 9. Decisión

**DEFER — laboratorio obligatorio.** Existe un camino oficial con límites de empaquetado conocidos, pero no hay evidencia empírica de viabilidad de NotaLocal en su hardware. No se inicia una decisión final de R-2/R-3/R-4 hasta registrar una corrida exitosa o un fallo reproducible con pins exactos.
